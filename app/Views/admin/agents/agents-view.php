...
<div class="w-full mx-auto">
  <div x-data="AgentsTable('<?= csrf_hash() ?>')" class="flex flex-col border min-w-full border-gray-200 rounded-lg px-5 py-5 mx-auto min-h-[calc(100vh-250px)]">
    ...
    <div class="mb-5 flex justify-between">
      ...
      <a href="/admin/agents/create" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg bg-primary-950 border border-primary-line text-primary-foreground hover:bg-primary-hover focus:outline-hidden focus:bg-primary-hover disabled:opacity-50 disabled:pointer-events-none" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-governance-form-modal" data-hs-overlay="#hs-governance-form-modal">
        Add
      </a>
    </div>
          ...
          <table id="users-table" class="min-w-full h-full divide-y divide-gray-200">
            <thead>
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Middle Name</th>
            ...
            <tbody>
              ...
              <template x-if="!loading">
                <template x-for="row in data" :key="row.agent_no">
                  <tr class="odd:bg-white even:bg-gray-100">
                    <td class="px-2 py-3 whitespace-nowrap">
                      <label class="relative inline-block w-11 h-6 cursor-pointer">
                        <input @click="toggleActive(row.agent_no, $event.target.checked)" :checked="!Boolean(parseInt(row.agent_inactive))" type="checkbox" class="peer sr-only">
                    ...
                    <td x-text="row.agent_fname" class="px-6 py-3 whitespace-nowrap text-sm text-gray-800"></td>
                    <td x-text="row.agent_mname" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800"></td>
                    <td x-text="row.agent_lname" class="px-6 py-3 whitespace-nowrap text-sm text-gray-800"></td>
                    <td x-text="row.agent_contact" class="px-6 py-3 whitespace-nowrap text-sm text-gray-800"></td>
                    <td x-text="row.agent_email" class="px-6 py-3 whitespace-nowrap text-sm text-gray-800"></td>
                    <td class="px-6 py-3 whitespace-nowrap text-end text-sm font-medium">
                      <div class="inline-flex gap-x-2">
                        <a x-bind:href="`/admin/agents/picture/${row.agent_no}`" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-neutral-600 hover:text-blue-500 focus:outline-hidden del-btn">
                          <i class="fa-solid fa-camera"></i>
                        </a>
                        <a x-bind:href="`/admin/agents/edit/${row.agent_no}`" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-neutral-600 hover:text-orange-400 focus:outline-hidden">
