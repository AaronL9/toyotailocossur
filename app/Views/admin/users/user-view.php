...
<div class="w-full mx-auto">
  <div x-data="UsersPage()" class="flex flex-col border min-w-full border-gray-200 rounded-lg px-5 py-5 mx-auto">
    ...
    <div class="mb-5 flex justify-between">
      ...
      <a href="<?= base_url('admin/users/create') ?>" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg bg-primary-950 border border-primary-line text-primary-foreground hover:bg-primary-hover focus:outline-hidden disabled:opacity-50 disabled:pointer-events-none">
        Add
      </a>
    </div>
    <!-- End Header -->
          ...
          <table id="users-table" class="min-w-full h-full divide-y divide-gray-200">
            <thead class="bg-gray-50 sticky top-0 z-10">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">User No.</th>
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
              ...
              <template x-if="!loading">
                <template x-for="user in users" :key="user.user_no">
                  <tr class="odd:bg-white even:bg-gray-100" x-bind:data-id="user.user_no">
                    ...
                    <td x-text="user.user_no" class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800"></td>
                    <td x-text="`${user.user_fname} ${user.user_mname ? user.user_mname + ' ' : ''}${user.user_lname}`" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800"></td>
                    <td x-text="user.user_uname" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800"></td>
